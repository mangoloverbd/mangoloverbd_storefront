...
function IngredientRail() {
  return <ul className="no-scrollbar -mx-4 flex snap-x snap-mandatory gap-5 overflow-x-auto scroll-smooth px-5 pb-2 scroll-px-5 sm:hidden">{ingredients.map((ingredient, index) => <li key={ingredient.name} className="min-w-[78%] snap-start border-y border-[#5b3b18]/20 py-4"><span className="block font-serif text-5xl font-medium leading-none tracking-[-0.08em] text-[#d99a2b]/70">{String(index + 1).padStart(2, "0")}</span><article className="mt-4 text-center"><div className="mx-auto aspect-square max-w-[190px] overflow-hidden rounded-full border-4 border-[#f0d2a0] bg-white shadow-sm"><img src={`/step/honey-nut/${ingredient.image}`} alt={ingredient.name} width="400" height="400" loading="lazy" className="h-full w-full object-cover" /></div><h3 className="honey-nut-heading mt-3 text-xl text-[#3d211a]">{ingredient.name}</h3><p className="mt-1 text-xs font-semibold uppercase tracking-[0.08em] text-[#a16a12]">{ingredient.category}</p></article></li>)}</ul>;
}
