...
export function HoneyNutCheckout({ product, status, productQuery, inventoryQuery, onRetry }: HoneyNutCheckoutProps) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const payload = {
        ...buildHoneyNutOrderPayload({ productName: refreshedProduct.name, pack: freshPack, quantity, customerName: name, phone, address: buildHoneyNutAddress(address) }),
        ...(draftKey ? { draftKey } : {}),
      };
