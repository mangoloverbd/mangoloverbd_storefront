...
test("renders product reels as a smooth horizontal snap carousel", () => {
  ...
  assert.match(productSource, /src=\{honeyNutReelEmbeds \? HONEY_NUT_REEL_POSTERS\[i\] : poster\}[\s\S]*?pointer-events-none/);
...
test("uses the new Cloudinary reels only for Honey Nut", () => {
  assert.match(productSource, /const HONEY_NUT_REEL_EMBEDS = \[/);
  assert.match(productSource, /FDown\.vn_Facebook_Video_Downloader_720p_HD__7925/);
  assert.match(productSource, /snapsave-app_1432224402135483_hd/);
  assert.match(productSource, /FDown\.vn_Facebook_Video_Downloader_720p_HD__8e09/);
  assert.match(productSource, /player%5Bshow_logo%5D=false/);
  assert.match(productSource, /const honeyNutReelEmbeds = slug === "honey-nut" \? HONEY_NUT_REEL_EMBEDS : null/);
  assert.match(productSource, /honeyNutReelEmbeds\[i\]/);
  assert.match(productSource, /const HONEY_NUT_REEL_POSTERS = \[/);
  assert.match(productSource, /video\/upload\/so_1,w_480,f_auto,q_auto/);
  assert.match(productSource, /honeyNutReelEmbeds \? HONEY_NUT_REEL_POSTERS\[i\] : poster/);
  assert.match(productSource, /honeyNutReelEmbeds && i !== currentReel/);
  assert.match(productSource, /allow="autoplay; fullscreen; encrypted-media; picture-in-picture"/);
  assert.match(productSource, /allowFullScreen/);
